...
function AdminReportDetailsPage() {
  ...
  const bottomRef = useRef(null); // kept if you still want an anchor node; not required for scrolling
  const [toastMessage, setToastMessage] = useState('');
  const [shouldScrollToBottom, setShouldScrollToBottom] = useState(false);
  const [initialScrolled, setInitialScrolled] = useState(false); // ensure initial instant scroll only once
  ...
  // --- INITIAL: scroll the *log container* to bottom once after first load (instant, no page jump)
  useEffect(() => {
    if (isLoading) return; // wait until loading finishes
    if (initialScrolled) return; // already done
    const container = logContainerRef.current;
    if (!container || !report?.communicationLog || report.communicationLog.length === 0) {
      setInitialScrolled(true); // mark done even if empty
      return;
    }

    // wait until browser paints messages, then set container scrollTop to bottom
    requestAnimationFrame(() => {
      // small timeout to be safer on slower renders
      setTimeout(() => {
        try {
  ...
  }, [isLoading, report?.communicationLog?.length, initialScrolled]);

  // --- ON NEW MESSAGES: always smoothly scroll the log container to bottom ---
  useEffect(() => {
    // don't trigger smooth scroll during the initial instant scroll
    if (!initialScrolled) return;
    const container = logContainerRef.current;
    if (!container || !report?.communicationLog) return;

    // If there are no messages, nothing to do
    if (report.communicationLog.length === 0) return;

    // Defer until DOM updated
    requestAnimationFrame(() => {
      // small timeout to ensure the newest message is rendered before scrolling
      setTimeout(() => {
        try {
  ...
  }, [report?.communicationLog?.length, initialScrolled]);
  ...
  const handlePostMessage = async (e) => {
    ...
    try {
      ...
      setNewMessage('');
      // no need to set a flag — the onSnapshot will update report and trigger the smooth scroll effect
    } catch (error) {
            ...
            <div className="bg-white shadow-xl rounded-2xl overflow-hidden">
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div><label className="text-sm font-medium text-gray-500">Date Submitted</label><p className="text-gray-900 font-medium">{report?.createdAt ? new Date(report.createdAt.seconds * 1000).toLocaleString() : ''}</p></div>
                    ...
                    {images.length > 0 && (
                      <div className="mb-8">
                        <div className="flex items-center justify-between mb-4">
                          <h3 className="text-md font-medium text-gray-700">Images ({images.length})</h3>
                          {images.length > 1 && <button onClick={() => openImageModal(0)} className="text-sm text-blue-600 hover:text-blue-800 font-medium">View All</button>}
                        </div>
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                          {images.map((imageUrl, index) => (
                            <div key={index} className="group cursor-pointer transform hover:scale-105 transition-transform duration-200" onClick={() => openImageModal(index)}>
                              <div className="aspect-square bg-gray-100 rounded-lg overflow-hidden border-2 border-transparent group-hover:border-blue-500 transition-colors">
                                <img src={imageUrl} alt={`Evidence image ${index + 1}`} className="w-full h-full object-cover" />
                              </div>
              ...
              <div ref={logContainerRef} className="mt-6 space-y-4 max-h-96 overflow-y-auto p-2">
                {report?.communicationLog && report.communicationLog.length > 0 ? (
                  report.communicationLog.map((entry, index) => (
                    <div key={index} className={`flex flex-col ${ entry.authorId === currentUser?.uid ? 'items-end' : 'items-start' }`}>
                      <div className={`p-4 rounded-lg max-w-lg ${ entry.authorId === currentUser?.uid ? 'bg-blue-600 text-white' : entry.authorRole === 'system' ? 'bg-yellow-100 text-yellow-900' : 'bg-gray-200 text-gray-800' }`}>
                        <p className="text-sm font-semibold">
                          {entry.authorId === currentUser?.uid ? 'You' : entry.authorRole === 'user' ? `${entry.authorName} (${entry.authorType})` : entry.authorType === 'superAdmin' ? `${entry.authorName} - Head Administrator` : entry.authorRole === 'admin' && entry.department ? `${entry.authorName} - ${entry.department} Administrator` : (entry.authorName || (entry.authorRole ? entry.authorRole.charAt(0).toUpperCase() + entry.authorRole.slice(1) : ''))}
                        </p>
                        <p className="mt-1">{entry.message}</p>
                        <p className={`text-right text-xs mt-2 ${ entry.authorId === currentUser?.uid ? 'text-blue-100' : 'text-gray-500' }`}>
                          {entry.timestamp?.seconds ? new Date(entry.timestamp.seconds * 1000).toLocaleString() : new Date(entry.timestamp).toLocaleString()}
                        </p>
                      </div>
                    </div>
                  ))
                ...
                {/* optional anchor; not required for scrolling but kept for compatibility */}
                <div ref={bottomRef} />
              </div>
          ...
          <div className="space-y-8">
            <div className="bg-white shadow-xl rounded-2xl p-6">
              ...
              <div className="mt-6 space-y-6">
                <div>
                  <label htmlFor="statusSelect" className="text-sm font-bold text-gray-600 block mb-2">Update Status</label>
                  <select id="statusSelect" onChange={(e) => setSelectedStatus(e.target.value)} className="w-full p-3 border border-gray-300 rounded text-sm mb-3" value={selectedStatus}>
                    <option value={report?.status}>Current: {report?.status}</option>
                    {allowedStatuses.map(status => <option key={status} value={status}>Change to: {status}</option>)}
                  </select>
                  <button onClick={handleUpdateStatus} disabled={isUpdating || selectedStatus === report?.status} className="w-full px-4 py-3 bg-blue-600 text-white font-semibold rounded-md hover:bg-blue-700 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed">{isUpdating ? 'Updating...' : 'Update Status'}</button>
                  <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs font-medium text-gray-600">Workflow:</p>
                    <p className="text-xs text-gray-700 mt-1">{report?.status} {allowedStatuses.length > 0 && ` → ${allowedStatuses.join(' → ')}`}{allowedStatuses.length === 0 && ' (Final status)'}</p>
                  </div>
                </div>
